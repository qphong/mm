...
// example:     var a4 = createArrowOverlay(map, start, end,  "#FFFF00", 0.9, "New");
function createArrowOverlay(map, start, end, color, opacity, tooltip) {
    var mid = new google.maps.LatLng((start.lat() + end.lat()) / 2.0, (start.lng() + end.lng()) / 2.0);
    var rotation = Math.atan((end.lat() - start.lat()) / (end.lng() - start.lng())) * 180.0 / Math.PI;
    if (end.lng() - start.lng() < 0) rotation = 180 + rotation;
    rotation = 90 - rotation;
    while (rotation < 0) rotation += 360.0;
    return new ArrowOverlay(map, mid, rotation, color, opacity, tooltip);
}
